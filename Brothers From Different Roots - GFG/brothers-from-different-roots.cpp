...
bool fetch(Node*root2,int x)
{
    if(!root2) return false;
    if(x==root2->data) return true;
    int  z = false;
    int y =  false;
    if(x<root2->data)  z= fetch(root2->left,x);
    else if(x>root2->data)  y= fetch(root2->right,x);
    return z or y;
}

Node* solve(Node*root1,Node*root2,int x)
{
